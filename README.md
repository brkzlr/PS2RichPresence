...
## Requirements
The compilation requirements and building steps below are only needed when building from source. If your OS and architecture are covered on the [Releases](https://github.com/brkzlr/PS2RichPresence/releases) page, download the matching archive instead and skip to [Usage](#usage).

- CMake 3.22 or higher.
- C compiler with C11 support, plus a C++ compiler for discord-rpc.
  - Linux and macOS: a modern GCC or Clang toolkit and `make`.
  - Windows: Visual Studio 2022 or its Build Tools with the "Desktop development with C++" workload.
- Git.
- Docker or ps2dev is required only to build the PS2 ELF launcher.
- Discord-RPC library.
...
### Building
- On Linux or macOS, run `compile.sh` after cloning the repository. It builds discord-rpc, the PC receiver, the title database copy and the PS2 disc launcher. Use `./compile.sh --skip-ps2` if you only want the PC receiver.
```
git clone --recursive https://github.com/brkzlr/PS2RichPresence
cd PS2RichPresence
./compile.sh
```
- On Windows, run `.\compile.ps1` instead. It builds the x64 PC receiver in `bin`. Use `.\compile.ps1 -Clean` for a clean rebuild. Keep `discord-rpc.dll` beside `ps2rpc.exe`.

On Linux or macOS, you can run `sudo cmake --install build` after `./compile.sh` to install PS2RichPresence to `/opt/PS2RichPresence`.
...
The PS2 disc launcher needs the ps2dev toolchain. `compile.sh` uses a local ps2dev install when available, otherwise it builds the ELF through Docker, which writes `bin/ps2rp.elf`.
...
- `/opt/PS2RichPresence` on Linux or macOS if you ran the installation command in the previous section.
```
./ps2rpc -a (Discord application ID) [-p Announce port] [-d title_db.tsv] [-u title_overrides.tsv] [-A image_asset_key] [-C]
```
On Windows, use `.\ps2rpc.exe` with the same options.
...
- On Windows, allow `ps2rpc.exe` through Windows Defender Firewall for private networks. The default inbound UDP port is `50003`.
